...
namespace Library2
{
    ...
    internal class Library
    {
        ...
        public static void InsertAuthor(int id, string last_name, string firs_name)
        {
            string cmd = "INSERT Authors(author_id, last_name, firs_name) VALUES(@id, @last_name, @firs_name)";
            SqlCommand command = new SqlCommand(cmd, connection);
            ...
            SqlParameter p_firs_name = new SqlParameter("@firs_name", SqlDbType.NVarChar, 150); p_firs_name.Value = firs_name;

            command.Parameters.Add(p_id);
            command.Parameters.Add(p_last_name);
            command.Parameters.Add(p_firs_name);
        ...
        {
            string cmd =
                "SELECT author_id FROM Authors WHERE last_name=@last_name AND firs_name=@firs_name";
            SqlCommand command = new SqlCommand(cmd, connection);
            command.Parameters.Add("@firs_name", SqlDbType.NVarChar, 150).Value = full_name.Split(' ').First();
